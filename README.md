...
* echttp_jsonget loads the JSON data from the file name (first argument) and prints the value associated with each JSON path provided (subsequent arguments). Note that the JavaScript array syntax requires quoting in shell.

Both tools have minimal features. They were intended to test the JSON parser, but can be useful to analyze the content of a JSON file, especially when the JSON data was not formatted for readability.
