...
A shorthand for setting the Content-Type attribute to text/plain.
...
Declare a handler that is called when the target URI is not found. This gives the application a chance to create the target file on the fly. This function returns the previous handler, allowing the application to chain handlers, if needed.

The handler must return a file descriptor to the file at the given path, or -1 if the file cannot be created.
