...
The echttp library includes a small JSON parser, built with the same philosophy as echttp itself: make the API simple to use. There are also two JSON utilities provided with echttp:
* echttp_jsonprint reformats the content of the JSON files provided and prints that reformated data.
* echttp_jsonget load the JSON data from the file name (first argument) and print the value associated with ach JSON path provided (subsequent arguments). If the JSON data starts with an anonymous object, the path must start with a '.'; if the JSON data starts with an anonymous array, the path must start with an array index.
...
```
Search a specific JSON item in the list of tokens generated by a previous parsing. Returns the index of the element identified by the path, or -1 on failure. If the JSON data starts with an anonymous object, the path must start with a '.'; if the JSON data starts as an anonymous array, the path must start with an array index.

Its is valid to provide a path that ends on an object or array. Examples of valid path (matching the content of test/test.json):
```
    .rawobject.real
    .array[6].item2
    .rawobject.object
